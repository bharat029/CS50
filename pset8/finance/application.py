...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE uid = :user_id GROUP BY symbol HAVING total_shares > 0", user_id=session["user_id"])
    quotes = {}
...
def buy():
    ...
    if request.method == 'POST':
        ...
        elif not len(request.form.get("symbol")):
            return apology("must provide symbol", 403)
        ...
        db.execute("INSERT INTO transactions (uid, symbol, shares, price_per_share) VALUES(:user_id, :symbol, :shares, :price)",
...
def history():
    ...
    transactions = db.execute(
        "SELECT symbol, shares, price_per_share, created_on FROM transactions WHERE uid = :user_id ORDER BY created_on", user_id=session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure password was submitted
            ...
            return apology("confirmation and password must match", 400)
...
def sell():
    ...
    if request.method == "POST":
        ...
        stock = db.execute("SELECT SUM(shares) as total_shares FROM transactions WHERE uid = :user_id AND symbol = :symbol GROUP BY symbol",
